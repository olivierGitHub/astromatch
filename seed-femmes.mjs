...
const PHOTOS_DIR = join(import.meta.dirname, 'mock-data/femmes');
const NGROK_HEADERS = { 'ngrok-skip-browser-warning': 'true' };

const PROFILES = [
  { birthDate: '1995-06-14', birthPlace: 'Paris, France', lat: 48.8566, lng: 2.3522, tz: 'Europe/Paris', location: 'Paris, France', bio: 'Passionnée d\'astrologie et de voyages ✨' },
  { birthDate: '1992-03-22', birthPlace: 'Lyon, France', lat: 45.7640, lng: 4.8357, tz: 'Europe/Paris', location: 'Lyon, France', bio: 'Yoga, thé et bonne musique.' },
  { birthDate: '1998-11-05', birthPlace: 'Paris, France', lat: 48.8566, lng: 2.3522, tz: 'Europe/Paris', location: 'Paris, France', bio: 'Balance ascendant Scorpion, cherche quelqu\'un de sincère.' },
  { birthDate: '1990-08-30', birthPlace: 'Montreal, Canada', lat: 45.5017, lng: -73.5673, tz: 'America/Montreal', location: 'Montreal, Canada', bio: 'Entre deux continents, en quête de connexion vraie.' },
  { birthDate: '1996-01-17', birthPlace: 'Lyon, France', lat: 45.7640, lng: 4.8357, tz: 'Europe/Paris', location: 'Lyon, France', bio: 'Artiste et rêveuse. Les étoiles me guident.' },
  { birthDate: '1993-09-09', birthPlace: 'New York, USA', lat: 40.7128, lng: -74.0060, tz: 'America/New_York', location: 'New York, USA', bio: 'Curieuse, directe, et fan de jazz.' },
  { birthDate: '1997-04-25', birthPlace: 'Paris, France', lat: 48.8566, lng: 2.3522, tz: 'Europe/Paris', location: 'Paris, France', bio: 'Capricorne qui aime les longues conversations.' },
  { birthDate: '1991-12-03', birthPlace: 'Lyon, France', lat: 45.7640, lng: 4.8357, tz: 'Europe/Paris', location: 'Lyon, France', bio: 'Nature, livres et étoiles filantes.' },
];
...
const DYNAMICS_OPTIONS = [
  ...
  ['spiritual_alignment'],
  ['friendship_first'],
  ['deep_connection', 'adventure_together'],
  ['playful_exploration'],
  ['passion_forward'],
  ['co_creation'],
];
...
async function createAccount(photo, index) {
  const email = `femme${index + 1}@mock.com`;
  ...
  await api('/api/v1/me/profile/identity', {
    method: 'PUT',
    headers: auth,
    body: JSON.stringify({ gender: 'FEMALE', attraction: 'MEN' }),
  });
  ...
  form.append('file', new Blob([photoBytes], { type: mime }), photo);
...
async function main() {
  ...
  console.log(`${files.length} photos trouvées dans mock-data/femmes/`);
