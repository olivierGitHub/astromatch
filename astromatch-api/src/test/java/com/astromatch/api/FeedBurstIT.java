...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, properties = {
		"astromatch.feed.swipe-burst-per-minute-per-user=2" })
class FeedBurstIT {
	...
	@Test
	void thirdSwipeInOneMinuteReturns429() throws Exception {
		String accessV = onboard("burst_v@example.com");
		onboard("burst_t1@example.com");
		onboard("burst_t2@example.com");
		onboard("burst_t3@example.com");
		...
		assertThat(swipe(accessV, ids.get(0), "PASS").statusCode()).isEqualTo(200);
		assertThat(swipe(accessV, ids.get(1), "PASS").statusCode()).isEqualTo(200);
		var third = swipe(accessV, ids.get(2), "PASS");
		assertThat(third.statusCode()).isEqualTo(429);
		assertThat(third.body()).contains("RATE_LIMITED");
		assertThat(third.body()).contains("traceId");
	}
