...
class MatchIT {
	...
	@Test
	void mutualLikeCreatesMatchOnSecondSwipe() throws Exception {
		String accessA = onboard("match_it_a@example.com");
		String accessB = onboard("match_it_b@example.com");
		...
		var swipeA = postJsonAuth("/api/v1/feed/swipe", accessA,
				"{\"targetUserId\":\"" + idB + "\",\"action\":\"LIKE\"}");
		assertThat(swipeA.statusCode()).isEqualTo(200);
		assertThat(swipeA.body()).contains("\"match\":null");

		var swipeB = postJsonAuth("/api/v1/feed/swipe", accessB,
				"{\"targetUserId\":\"" + idA + "\",\"action\":\"LIKE\"}");
		assertThat(swipeB.statusCode()).isEqualTo(200);
		assertThat(swipeB.body()).contains("\"matchId\"");
		assertThat(swipeB.body()).contains(idA);

		var matchesB = getAuth("/api/v1/matches", accessB);
		assertThat(matchesB.statusCode()).isEqualTo(200);
		assertThat(matchesB.body()).contains(idA);
	}

	@Test
	void messagesRoundTrip() throws Exception {
		String accessA = onboard("msg_it_a@example.com");
		String accessB = onboard("msg_it_b@example.com");
		...
		var send = postJsonAuth("/api/v1/matches/" + matchId + "/messages", accessB,
				"{\"body\":\"Hello from B\"}");
		assertThat(send.statusCode()).isEqualTo(200);
		assertThat(send.body()).contains("Hello from B");

		var list = getAuth("/api/v1/matches/" + matchId + "/messages", accessA);
		assertThat(list.statusCode()).isEqualTo(200);
		assertThat(list.body()).contains("Hello from B");
	}
